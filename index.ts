import { Client, Guild, Intents, TextChannel, VoiceChannel } from "discord.js"
...
const consts = {
  ...
  GENERAL_CHANNEL_ID: "265182824082964481",
...
api.get("/users", async () => {
  const vc = (await client.channels.fetch(
    consts.GENERAL_CHANNEL_ID
  )) as VoiceChannel
  if (!vc) return { users: [], count: 0, err: "vc not found" }

  return {
    users: vc.members.map(member => member.user.username),
